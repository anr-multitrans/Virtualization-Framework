...
from Synchro4 import run_carla_simulation
...
class SimulationThread(threading.Thread):
    def __init__(self, rgb_label, semantic_label, instance_label, length=100000, progress_bar=None, ThreeD=True, window=None):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            post_processing_thread = PostProcessingThread('images', 'environment_object.json', self.window.selected_labels)
            #post_processing_thread.start()

            #close_carla_server()
class PostProcessingThread(threading.Thread):
    def __init__(self, batch_folder, catalog_json_filename,selected_labels=None):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            # Run your CARLA simulation code here
             post_process(self.batch_folder, self.catalog_json_filename,self.selected_labels)
        except Exception as e:
            # Handle any exceptions that might occur during simulation
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.vehicle = None
        self.default_scenario_length=100000
        self.default_cc_scenario_length=200
        self.scenario_length = 100000
        self.x = 0
    ...
    def isRunning(self):
        to_return=False
        try:
            to_return= self.is_running
        except e:
            to_return=False 
        return to_return
    def init_ui(self):

        self.folder_button = QPushButton('Change directory')
    ...
    def setup_layout(self, output_layout, main_bb_vbox, main_cc_vbox, progress_layout, recording_widget, palette_widget,
        ...
        self.label_rgb.setMinimumSize(100, 100)
        self.label_seg.setMinimumSize(100, 100)
        self.label_bounding.setMinimumSize(100, 100)
        self.label_rgb.setFixedSize(1200, 800)
        self.label_seg.setFixedSize(1200, 800)
        self.label_bounding.setFixedSize(1200, 800)
        
        layout.addWidget(self.label_rgb, 0, 0,1,2)
        layout.addWidget(self.label_seg, 0, 2,1,2)
        layout.addWidget(self.label_bounding, 0, 4,1,2)
        layout.addLayout(output_layout, 1, 0, 1,3)
        layout.addLayout(main_bb_vbox, 1, 3,2,1)
        layout.addLayout(main_cc_vbox, 1, 4,2,2)
        #layout.addLayout(progress_layout, 1, 1)
        #layout.addWidget(recording_widget, 2, 1, 1, 1)
        layout.addWidget(palette_widget, 3, 0, 1, 2)
        layout.addLayout(info_layout, 3, 2)
        layout.addWidget(group_box, 3,4)
    ...
    def toggle_scenario_selection(self, checked):
        if checked:
            ...
            self.scenario_length = self.default_scenario_length
        else:
            ...
            self.scenario_length = self.default_cc_scenario_length
    ...
    def setup_palette(self):
        ...
        palette_layout.addWidget(self.rec_btn)
    ...
    def start_scenario(self):
        ...
        self.simulation_thread = SimulationThread(
            self.label_rgb, self.label_seg, self.label_bounding, self.scenario_length, self.progress_bar,self.ThreeD,self
        )
    ...
    def update_rgb_label_slot(self, image):
        ...
        self.label_rgb.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def update_semantic_label_slot(self, image):
        ...
        self.label_seg.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def update_instance_label_slot(self, image):
        ...
        self.label_bounding.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.label_bounding.setAlignment(Qt.AlignCenter)
...
if __name__ == '__main__':
    ...
    try:
        ...
        app.setStyleSheet("""
        QLabel, QPushButton, QLineEdit, QRadioButton, QCheckBox, QGroupBox, QComboBox, QListWidget {
            font-family: Arial;
            font-size: 32px;
