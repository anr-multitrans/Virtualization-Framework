...
class MainWindow(QMainWindow):
    ...
    def spawn_vehicle_and_cameras(self):
        ...
        self.inst_seg_camera_bp.set_attribute('image_size_y', '%d' %self.sesor_height)
        ...
        self.inst_seg_camera.listen(lambda data: sensor_callback(self.world,self.inst_seg_camera, data,synchro_queue,"instance_segmentation",self.K))
...
#
    ...
    def timerEvent(self, event):
        ...
        if len(self.multiple_bbox_tags_colors) == 0:
            self.multiple_bbox_tags_colors  = [self.CLASS_MAPPING[label] for label in self.multiple_bbox_tags] 
        ...
        if self.is_running:
            ...
            try:
                ...
                for _ in range(len(self.synchro_list)):
                    ...
                    #print(s_frame[2] )
                    print(frame)
                if rgb_image!=None and boxes!=None and transform!=None and instance_image!=None and segmentation_image !=None :
                    ...
                    if frame%2==0:
                        if self.ThreeD:
                            ...
                            boxes = process_colors(rgb_image_draw, blended_image, sem_array, self.multiple_bbox_tags_colors, 5, 5,self.CLASS_MAPPING)
                            json_array=self.update_bounding_box_view_simple( self.rgb_camera, rgb_image_draw, segmentation_image, boxes, transform,rgb_ref)
                            #self.update_bounding_box_view_smart(self.rgb_camera,rgb_image,segmentation_image,boxes,transform,rgb_ref)
                            self.record_tick(json_array,rgb_ref,segmentation_image,rgb_image_draw)
                else:
    ...
    def update_bounding_box_view_simple(self, camera, img, semantic_image, bounding_box_set, transform,image_ref):
        json_array = []
    ...
    #[CLASS_MAPPING[label] for label in multiple_bbox_tags]
