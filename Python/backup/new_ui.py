...
class MainWindow(QMainWindow):
    ...
    def timerEvent(self, event):
        ...
        if len(self.multiple_bbox_tags_colors) == 0:
            self.multiple_bbox_tags_colors = [CLASS_MAPPING[label] for label in multiple_bbox_tags]
        ...
        if self.is_running:
            check_connection_status(self.client)
            ...
            for _ in range(len(self.synchro_list)):
                ...
                if s_frame[1] == "rgb_camera":
                    rgb_image = get_RGB_DATA(s_frame[2])
                if s_frame[1] == "rgb_camera_ref":
                    rgb_ref = update_rgb_camera_view(s_frame[2], self.label_rgb, self.desired_width,
                                                     self.desired_height)  # self.update_rgb_camera_view(s_frame[2])
                if s_frame[1] == "semantic_segmentation":
                    segmentation_image = update_seg_camera_view(s_frame[2], self.label_seg, self.desired_width,
                                                                self.desired_height)  # self.update_seg_camera_view(s_frame[2])
                if s_frame[1] == "instance_segmentation":
                    instance_image = s_frame[2]
                ...
                # print(s_frame[2] )
                print(frame)
            if rgb_image is not None and boxes is not None and transform is not None \
                    and instance_image is not None and segmentation_image is not None:
                instance_array, sem_array, rgb_image_draw, alpha, blended_image =  \
                    self.init_arrays(instance_image, segmentation_image, rgb_image)
                
                # print(rgb_image.width)
                if frame % 2 == 0:
                    if self.ThreeD:
                        json_array = update_bounding_box_view_3D(self.label_bounding, self.rgb_camera, rgb_image,
                                                                 segmentation_image, boxes, transform, rgb_ref,
                                                                 self.ImageCounter, self.desired_width,
                                                                 self.desired_height, self)
                        record_tick(self.data, json_array, rgb_ref, segmentation_image, rgb_image_draw, self.is_running,
                                    self.is_recording)
                    else:

                        boxes = process_colors(rgb_image_draw, blended_image, sem_array, self.multiple_bbox_tags_colors,
                                               5, 5, CLASS_MAPPING)
                        json_array = update_bounding_box_view_simple(self.label_bounding, self.rgb_camera,
                                                                     rgb_image_draw, segmentation_image, boxes,
                                                                     transform, rgb_ref, self.ImageCounter,
